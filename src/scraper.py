...
class Scraper(threading.Thread):
    def __init__(self, start_url, max_depth, extensions, db_name='files.db'):
        ...
        # ⚡ Bolt: Precompute extension tuple for O(1) C-level path.endswith matching
        # Reduces overhead from an O(N) loop during URL validation.
        self.extension_tuple = tuple(f".{ext}" for ext in self.extensions)
    ...
    def is_target_file(self, url):
        # ⚡ Bolt: Only lowercase the path once, instead of N times in a loop
        path_lower = urlparse(url).path.lower()

        # ⚡ Bolt: O(1) fast-path check using C-level tuple matching
        if path_lower.endswith(self.extension_tuple):
