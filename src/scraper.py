            ...
            try:
                ...
                if response.status_code in (301, 302, 303, 307, 308):
                    ...
                    if location and redirect_count < 5:
                        ...
                        clean_redir_url = parsed_redir.scheme + "://" + \
                            parsed_redir.netloc + parsed_redir.path
                        if parsed_redir.query:
                            clean_redir_url += "?" + parsed_redir.query

                        if is_safe_url(
                                clean_redir_url) and clean_redir_url not in self.visited:
                            self.queue.append(
                                (clean_redir_url, depth, redirect_count + 1))
                    continue
