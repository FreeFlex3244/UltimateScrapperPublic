...
class Scraper(threading.Thread):
    ...
    def crawl(self):
        ...
        while self.queue and not self.stop_event.is_set():
            ...
            try:
                # Fetch page securely without automatic redirects
                response = requests.get(url, timeout=10, allow_redirects=False)

                # Handle redirects manually to prevent SSRF
                if response.status_code in (301, 302, 303, 307, 308):
                    if redirect_count >= 5:
                        continue # Prevent infinite redirect loops (DoS)
                    location = response.headers.get('Location')
                    if location:
                        redirect_url = urljoin(url, location)
                        if is_safe_url(redirect_url) and redirect_url not in self.visited:
                            self.visited.add(redirect_url)
