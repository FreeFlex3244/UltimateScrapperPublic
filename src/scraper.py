    ...
    def crawl(self):
        ...
        while self.queue and not self.stop_event.is_set():
            ...
            try:
                ...
                # Handle redirects to prevent SSRF
                if response.status_code in (301, 302, 303, 307, 308):
                    ...
                    if location:
                        ...
                        parsed_full = urlparse(redirect_url)
                        clean_redirect = parsed_full.scheme + "://" + \
                            parsed_full.netloc + parsed_full.path
                        if parsed_full.query:
                            clean_redirect += "?" + parsed_full.query

                        if clean_redirect not in self.visited:
                            if is_safe_url(clean_redirect):
                                self.queue.append(
                                    (clean_redirect, depth, redirect_count + 1))
                            else:
                                print(
                                    f"Skipping unsafe redirect URL: {clean_redirect}")
                    continue
