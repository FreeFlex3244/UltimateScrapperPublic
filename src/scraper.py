            ...
            try:
                ...
                if response.status_code in (301, 302, 303, 307, 308):
                    ...
                    if location:
                        ...
                        clean_new_url = parsed_new.scheme + "://" + parsed_new.netloc + parsed_new.path
                        if parsed_new.query:
                            clean_new_url += "?" + parsed_new.query

                        if is_safe_url(clean_new_url) and redirect_count < 5:
                            if clean_new_url not in self.visited:
                                self.queue.append((clean_new_url, depth, redirect_count + 1))
                    continue
