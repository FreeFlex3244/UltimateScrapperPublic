...
class Scraper(threading.Thread):
    def __init__(self, start_url, max_depth, extensions, db_name='files.db'):
        ...
        # ⚡ Bolt: Precompute static values for inner loop performance
    ...
    def is_target_file(self, url):
        ...
        # ⚡ Bolt: Fast path using O(1) C-level tuple check before looping
        if path.lower().endswith(self.formatted_extensions):
            ...
            try:
                ...
                for link in links:
                    ...
                    if is_target:
                        ...
                        if depth < self.max_depth:
                            # ⚡ Bolt: Use precomputed netloc instead of parsing in every iteration
