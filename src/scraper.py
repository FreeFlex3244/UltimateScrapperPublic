...
class Scraper(threading.Thread):
    ...
    def crawl(self):
        ...
        while self.queue and not self.stop_event.is_set():
            ...
            try:
                # Fetch page securely preventing automatic redirects to avoid
                # SSRF
                response = requests.get(url, timeout=10, allow_redirects=False)

                # Handle manual redirects up to 5 times
                    ...
                    if redirect_count < 5:
                        ...
                        if location:
                            ...
                            if is_safe_url(clean_url):
                                self.queue.append(
                                    (clean_url, depth, redirect_count + 1))
                            else:
                                print(
                                    f"Skipping unsafe redirect URL: {clean_url}")
                    continue
