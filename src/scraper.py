...
class Scraper(threading.Thread):
    def __init__(self, start_url, max_depth, extensions, db_name='files.db'):
        ...
        self.formatted_extensions = tuple(f".{ext}" for ext in self.extensions) # Precompute for O(1) checks
        self.start_netloc = urlparse(start_url).netloc # Precompute to avoid redundant parsing
