...
class Scraper(threading.Thread):
    ...
    def run(self):
        ...
        # This reuses TCP connections for requests to the same host,
        # which significantly improves the performance of the web scraper.
        # It must be initialized here in run() to be thread-safe.
        ...
        try:
            ...
            self.db.close()
            # Clean up sockets from the connection pool
            self.session.close()
    ...
    def crawl(self):
        ...
        while self.queue and not self.stop_event.is_set():
            ...
            try:
                # Fetch page using the connection pool
