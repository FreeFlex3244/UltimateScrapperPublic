...
class Scraper(threading.Thread):
    def __init__(self, start_url, max_depth, extensions, db_name='files.db'):
        ...
        # Precompute a tuple of formatted extensions (e.g., ('.zip',
        # '.tar.gz')) for O(1) path.endswith matching
    ...
    def is_target_file(self, url):
        ...
        # O(1) C-level path.endswith using the precomputed tuple instead of
        # O(N) Python loop
        if path.endswith(self.ext_tuple):
