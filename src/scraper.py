...
class Scraper(threading.Thread):
    ...
    def crawl(self):
        ...
        while self.queue and not self.stop_event.is_set():
            ...
            try:
                # Fetch page - SSRF prevention: manually handle redirects
                response = requests.get(url, timeout=10, allow_redirects=False)

                # Manual redirect handling
                if response.status_code in (301, 302, 303, 307, 308):
                    new_url = response.headers.get('Location')
                    if new_url and redirect_count < 5:
                        full_new_url = urljoin(url, new_url)
                        if is_safe_url(full_new_url):
                            self.queue.append((full_new_url, depth, redirect_count + 1))
                            print(f"Following redirect to {full_new_url}")
                        else:
                            print(f"Blocked unsafe redirect to {full_new_url}")
                    continue
