...
class Scraper(threading.Thread):
    def __init__(self, start_url, max_depth, extensions, db_name='files.db'):
        ...
        # Precompute tuple of formatted extensions for O(1) C-level path
        # validation check
    ...
    def is_target_file(self, url):
        ...
        # Fast path O(1) C-level check using tuple before O(N) loop
        if not path.endswith(self.ext_tuple):
            return False, None
