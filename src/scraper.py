...
class Scraper(threading.Thread):
    def __init__(self, start_url, max_depth, extensions, db_name='files.db'):
        ...
        # Precompute formatted tuple for O(1) C-level path validation check
        self._extension_tuple = tuple(f".{ext}" for ext in self.extensions)
    ...
    def is_target_file(self, url):
        ...
        # O(1) C-level check to instantly skip non-matching files (Bolt
        # optimization)
        if not path.endswith(self._extension_tuple):
            return False, None
