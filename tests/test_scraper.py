...
class TestScraper(unittest.TestCase):
    ...
    def test_crawl_depth(self, mock_get):
        ...
        def side_effect(url, timeout=10, allow_redirects=False, **kwargs):
    ...
    def test_safe_redirect(self, mock_get):
        def side_effect(url, timeout=10, allow_redirects=False, **kwargs):
            ...
            if url == "http://example.com/redirect":
                ...
                mock_response.headers = {'Location': 'http://example.com/file.zip'}
            elif url == "http://example.com/file.zip":
                mock_response.status_code = 200
                mock_response.headers = {'Content-Type': 'text/html'}
                mock_response.content = b'<a href="file.zip">Download File</a>'
            return mock_response

        mock_get.side_effect = side_effect
        self.scraper.visited.clear()
        self.scraper.queue.append(("http://example.com/redirect", 0, 0))
        self.scraper.crawl()
        self.assertTrue(self.scraper.db.add_file.called)
